...
### Center Field
> You are the priority fielder on ALL fly balls you can reach — center fielder has priority over corner outfielders AND all infielders. Call it loud and early. Gap coverage: take angle routes, not straight-back routes. Do-or-die throws: charge the ball, crow-hop, throw through the cutoff. Communication is your responsibility — you see the whole field. Back up second base on infield plays. Your cutoff on throws home is the 1B. On doubles to the LEFT side (LF line, LF-CF gap): your relay is the SS. On doubles to the RIGHT side (RF-CF gap, RF line): your relay is the 2B.
...
MOUND VISIT LIMIT: Each team gets 5 mound visits per 9 innings (clock resets in extra innings). Catcher visits count toward this limit. If a team has no remaining visits, manager may still visit but MUST remove the pitcher. A visit to change signs counts as an official visit.
NEVER: A second mound visit to the same pitcher in the same inning without removing him. Bringing in a reliever who faces fewer than 3 batters (unless injury). Catcher warming up the new pitcher with the wrong hand. Using a mound visit to discuss the score or revisit a past mistake — visits are forward-looking only.
```
...
WHEN IBB IS JUSTIFIED:
  ...
  (3) Large platoon advantage vs the next hitter (vs the current hitter who has platoon edge) — the matchup swing must be >20 BA points to offset the RE24 cost. BRAIN.stats platoonEdge = 18 is the minimum floor.
...
| 9e | TAKING/SWINGING: Never present "always take" or "always swing" as absolute. All count decisions must be framed as situational | Absolute-framing error |
| 9f | SUN DEFENSE (OF scenarios): Glove shield is primary technique. Sunglasses are supplementary. Never present sunglasses as the only or first option | Sun technique inversion |
| 9g | CATCHER FRAMING: Subtle glove pull on borderline pitches; stillness in high-leverage counts. Never present as one-size-fits-all | Framing technique flattening |
